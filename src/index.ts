...
class DdServerApiByWeb {
  ...
  async requestT<T>(
    ...
    requestType?: 'form' | 'json' | 'fileData',
  ): Promise<T> {
    method ??= 'GET';
    ...
    let response = await this.client.request<T>({
      url: `${this.host}${url}`,
      method: method ?? 'GET',
      params: param,
      data: postData,
  ...
  async userList(pager: PageParam, user?: User): Promise<Result<{ page: PagerModel; list: User[] }>> {
    return this.requestT<Result<{ page: PagerModel; list: User[] }>>(
      '/api/user/list',
      Object.assign(user ?? {}, pager),
      'GET',
    );
  }
  ...
  async deleteBlog(blogId: number): Promise<Result<string>> {
    return this.requestT<Result<string>>(
   ...
   * @constructor
   */
  async getFilesWithFolderId(folderId: number, pageModel: PageParam): Promise<Result<Page<FileInfo>>> {
   ...
   */
  async getBlogList(page: number, pageSize: number): Promise<Result<BlogListData>> {
    return this.requestT<Result<BlogListData>>('/api/blog/list?page=' + page + '&pageSize=' + pageSize);
  }
   ...
   * @returns
   */
  async getTextByName(name: string, password: string = ''): Promise<Result<TextModel>> {
    return this.requestT<Result<TextModel>>(`/api/blog/text`, { password, name }, 'GET');
  }
  ...
  async register(loginName: string, password: string, pic: string): Promise<Result<undefined>> {
    return this.requestT<Result<undefined>>(
      '/api/auth/user-addnew',
      {
  ...
  async getResourceList(
    ...
    categoryId?: number,
    params?: any,
    paramsHandle?: (p: any) => void,
  ): Promise<Result<{ page: PagerModel; list: ResourceModel[] }>> {
   ...
   * @returns 查询结果
   */
  async findComment(params: SelectCommentParams): Promise<
    Result<{
      list: Comment[];
      page: PagerModel;
    }>
